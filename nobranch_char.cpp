...
vector<char> generate_chip_seq(int first_reg, int second_reg, int chip_sequence_length) {
    vector<char> chip_seq;
    chip_seq.reserve(chip_sequence_length);
    vector<char> registers_top(10, 1);
    vector<char> registers_bottom(10, 1);
    for (size_t i = 0; i < chip_sequence_length; i++) {
        char top_xor = registers_top[2] ^ registers_top[9];
        char previous_last_top = registers_top[9];
        registers_top[9] = top_xor;
        rotate(registers_top.begin(), registers_top.end() - 1, registers_top.end());
        char middle_xor_left = registers_bottom[first_reg - 1] ^ registers_bottom[second_reg - 1];
        chip_seq.push_back(previous_last_top ^ middle_xor_left);
        char bottom_xor = registers_bottom[1] ^ registers_bottom[2] ^ registers_bottom[5] ^ registers_bottom[7] ^ registers_bottom[8] ^ registers_bottom[9];
...
vector<char> read_signal(string filename, bool verbose) {
    vector<char> sum_signal;
...
vector<vector<char>> get_chip_sequences(int n_satellites, int chip_sequence_length, bool verbose) {
...
void chips_to_signals(vector<vector<char>>& chip_sequences, bool verbose) {
...
int dot_product(const vector<char>& chip_seq, const vector<char>& signal, unsigned int delta) {
    int product = 0;
    size_t size = chip_seq.size();
    size_t thresh = 828 * 0.6;
    size_t wrap = min(size - delta, thresh);
    size_t i = 0;
    for (; i < wrap; ++i) {
        product += chip_seq[i] * signal[i + delta];
...
void check_sequence(const vector<char>& sequence, const vector<char>& sum_signal, int satellite_nr) {
...
void check_sequences(vector<vector<char>>& sequences, const vector<char>& sum_signal) {
...
vector<double> run(int argc, char* argv[], vector<double>& sums) {
    ...
    vector<vector<char>> chip_sequences = get_chip_sequences(n_satellites, chip_sequence_length, false);
    auto t2 = high_resolution_clock::now();
    chips_to_signals(chip_sequences, false);
    auto t3 = high_resolution_clock::now();
    vector<char> sum_signal = read_signal(filename, false);
...
int main(int argc, char* argv[]) {
    ...
    cout << "n_runs = " << n_runs << "\tget_chip_sequences\tchips_to_signals\tread_signal\t\tcheck_sequences\t\tTotal" << endl;
    cout << "Total:\t\t";
    for (auto i : sums)
        cout << i << "ms\t\t";
    cout << endl << "Average:\t";

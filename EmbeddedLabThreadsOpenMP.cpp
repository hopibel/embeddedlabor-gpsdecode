...
void check_sequence(const vector<int>& sequence, const vector<int>& sum_signal, int idx) {
    // Schwellwert 1023 -3*65 = 828
    float factor = 0.5f;
    for (unsigned int j = 0; j < sequence.size(); j++) {

        int product = dot_product(sequence, sum_signal, j, factor);
...
void check_sequences(vector<vector<int>>& sequences, const vector<int>& sum_signal) {
    ...
    for (unsigned int i = 0; i < n_sequences; i++) {
        check_sequence(sequences[i], sum_signal, i);
    }
    // cout << "\n";
}
...
int main(int argc, char* argv[]) {
    ...
    const int processor_count = omp_get_max_threads() / 2;
    omp_set_num_threads(processor_count);
    cout << "\nThreads used: " << processor_count << endl;
    cout << "\n\nn_runs = " << n_runs << "\tget_chip_sequences\tchips_to_signals\tread_signal\tcheck_sequences" << endl;
    cout << "Total:\t\t";
    for (auto i : sums)
        cout << i << "ms\t\t";
    cout << endl << "Average:\t";

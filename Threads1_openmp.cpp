...
vector<vector<int>> get_chip_sequences(int n_satellites, int chip_sequence_length, bool verbose) {
    ...
    for (int i = 0; i < n_satellites; i++) {
        ...
        //Langsamer mit Async (In die for-Schleife muss i+=4 statt i++)
        //auto future1 = async(generate_chip_seq, first_regs[i], second_regs[i], chip_sequence_length);
        //auto future2 = async(generate_chip_seq, first_regs[i+1], second_regs[i+1], chip_sequence_length);
        //auto future3 = async(generate_chip_seq, first_regs[i+2], second_regs[i+2], chip_sequence_length);
        //auto future4 = async(generate_chip_seq, first_regs[i+3], second_regs[i+3], chip_sequence_length);
        //sequences.push_back(future1.get());
        //sequences.push_back(future2.get());
        //sequences.push_back(future3.get());
        //sequences.push_back(future4.get());
...
int dot_product(vector<int> chip_seq, vector<int> signal, unsigned int delta) {
    int product = 0;
    size_t size = chip_seq.size();
    size_t wrap = size - delta;
    size_t i = 0;
    for (; i < wrap; ++i) {
...
void check_sequence(vector<int> sequence, vector<int> sum_signal, int satellite_nr) {
...
void check_sequences(vector<vector<int>> sequences, vector<int> sum_signal) {
